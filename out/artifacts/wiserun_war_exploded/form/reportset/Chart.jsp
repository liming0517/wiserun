...
<div  style="height:100%; width:100%">
	...
	<div id="chartdiv1" align="center" style="height:88%; width:100%"></div>
</div>
      <script type="text/javascript">
      		var chartsql="";
      		var chartsx="";
			...
			function Line2D(charts){
				var chart = new FusionCharts("${pageContext.request.contextPath}/script/FusionCharts/Charts/Line.swf", "ChartId", ""+(document.all.chartdiv1.offsetWidth*0.95)+"", ""+(document.all.chartdiv1.offsetHeight*0.95)+"", "0", "0");
	     		chart.setDataXML(charts);
	     		chart.setTransparent(true);
	     		chart.render("chartdiv1");
			}
			...
			function MSColumn2D(charts){
				var chart = new FusionCharts("${pageContext.request.contextPath}/script/FusionCharts/Charts/MSColumn2D.swf", "ChartId", ""+(document.all.chartdiv1.offsetWidth*0.95)+"", ""+(document.all.chartdiv1.offsetHeight*0.95)+"", "0", "0");
	     		chart.setDataXML(charts);
	     		chart.setTransparent(true);
	     		chart.render("chartdiv1");
			}
			...
			function Pie3D(charts){
				var chart = new FusionCharts("${pageContext.request.contextPath}/script/FusionCharts/Charts/Pie3D.swf", "ChartId", ""+(document.all.chartdiv1.offsetWidth*0.95)+"", ""+(document.all.chartdiv1.offsetHeight*0.95)+"", "0", "0");
	     		chart.setDataXML(charts);
	     		chart.setTransparent(true);
	     		chart.render("chartdiv1");
			}
			...
			function doToolbar(id){
				if(id=="ok"){
					...
					$.ajax({
						...
						async:false,
						data: "active2=shuaxin&chartsql=" + chartsql+"&chartsx="+chartsx,  // 设置模式，一般为view ，主关键字值		
						dataType: 'text',
