function PopupCalendar(InstanceName, startYear, endYear)
{
	// ************edited by yangyiheng***********
	...
	this.currDate=new Date(this.endYear, 1, 1 );
	this.today=new Date();
	///Css
...
function CalendarInit()				///Create panel
{
	...
	{
		if(i==this.endYear) 
			htmloYear+="<option value='"+i+"' selected>"+i+"</option>";
		else
	...
	this.Fill();	
	// document.getElementById(this.sMONTHID).selectedIndex=sMonth-1;
}
...
function CalendarCellsMsOver(event, oInstance)				/// Cell MouseOver
{
...
function CalendarCellsMsOut(event, oInstance)				////// Cell MouseOut
{
	if(window.event) event = window.event;
	var myCell = (oInstance.isN ? event.target : event.srcElement);
...
function CalendarCellsClick(oCell,oInstance)
{
	...
	sMonth=oInstance.currDate.getMonth();
	sDay=oInstance.currDate.getDate();
	var inner = (oInstance.isN ? oCell.innerHTML : oCell.innerText);
	if(inner.substring(0,1)=="<") {
		// alert(inner.match(/^\d{1,2}$/)[0]);
		inner = inner.match(/\d{1,2}/)[0];
	}else if(inner.substring(0,1)=="&") {
		inner = " ";
	}
	if(inner!=" ")
	{
...
function getDateString(oInputSrc,oInstance)
{
	if(oInputSrc&&oInstance) 
	{
		...
		// CalendarDiv.style.pixelLeft=CalendargetPos(oInputSrc,"Left") + oInputSrc.offsetWidth; // ;
		CalendarDiv.style.left=CalendargetPos(oInputSrc,"Left") + oInputSrc.offsetWidth; // ;
